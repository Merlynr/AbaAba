...
class DQNAgent:
    def __init__(self):
        ...
        self.linear_annealed = (self.initial_epsilon - self.min_epsilon) / 2100
        ...
        if self.load_model:
            self.model = keras.models.load_model('./model/Dueling_Double_DQN/D3QN.h5')
            self.target_model = keras.models.load_model('./model/Dueling_Double_DQN/D3QN.h5')
        else:
    ...
    def create_model(self):
        ...
        fc1 = tf.keras.layers.Dense(32, activation='relu')(inputs)
...
log_dir = 'logs/dueling_ddqn/' + current_time
...
if agent.isTraining:
    for episode in track(range(episodes)):
        ...
        if (episode + 1) % 50 == 0:
            ...
            if agent.save_model:
                agent.model.save('./model/Dueling_Double_DQN/D3QN.h5')
                print('model saved')
            if agent.save_graph:
                agent.draw(episode, episode_rewards, average_rewards, "./model/Dueling_Double_DQN/D3QN_1.png")
        if (episode + 1) % 100 == 0:
            end_time = time.time()
