...
class DQNAgent:
    def __init__(self):
        ...
        if self.load_model:
            self.model = keras.models.load_model('./model/Double_DQN/DDPG_model_sample_db.h5')
...
if agent.isTraining:
    ...
    for episode in track(range(episodes)):
        ...
        if (episode + 1) % 50 == 0:
            ...
            if agent.save_model:
                agent.model.save('./model/Double_DQN/DDPG_model_sample_db.h5')
                print('model saved')
            if agent.save_graph:
                agent.draw(episode, episode_rewards, average_rewards, "./model/Double_DQN/DDPG_sample_db.png")
        if (episode + 1) % 100 == 0:
            end_time = time.time()
